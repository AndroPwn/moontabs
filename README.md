...
Moontabs discards inactive tabs to free up memory — but unlike other tab managers, it learns your browsing patterns over time.
...
```js
let INACTIVITY_LIMIT = 5 * 60 * 1000;  // minutes before a tab is discarded
let MAX_GAP  = 8 * 60 * 60 * 1000;     // gaps longer than this are ignored when learning
let MIN_GAP  = 30 * 1000;              // gaps shorter than this are ignored when learning
```

Exceptions (domains that are never discarded) can be managed from the popup without touching any code.
...
Contact: andromedapwn@gmail.com
